...
iteration_id <- "splines_study_0006"
initial_seed <- 457206

bg_colour <- "#ebf2fa"
bg_splines_colour1 <- "#D6E3EF" # a shade darker than bg_colour
bg_splines_colour2 <- "#427AA1" # a few shades darker again
...
# Splines to draw
splines1 <- draw_splines(
  seed_num = seed_vec[1], n_splines = n_splines_drawn)

splines2 <- draw_splines(
  seed_num = seed_vec[2], n_splines = n_splines_drawn)
...
p <- ggplot() +
  ...
  geom_bspline(
    ...
    colour = bg_splines_colour1, size = 0.2) +
  # Dots
  geom_point(
    data = dots,
    aes(x = x, y = y),
