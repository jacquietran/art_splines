...
splines_data1 <- throw_shapes(
  seed_num = seed_vec[1], n_shapes = n_shapes_filled, palette = colour_vec1)

set.seed(seed_vec[2])
splines_data2 <- throw_shapes(
  seed_num = seed_vec[2], n_shapes = n_splines_drawn, palette = colour_vec2) %>%
  mutate(
    spline_size = sample(
      seq(0.05, 0.6, by = 0.05), n(), replace = TRUE,
      prob = c(
        rep(0.1, times = 8),
        rep(0.05, times = 4))))
...
rect_w_subsets <- rect_data %>%
  distinct(group) %>%
  mutate(
    subset = sample(1:4, n(), replace = TRUE,
...
p <- ggplot() +
  ...
  ggfx::with_mask(
    geom_bspline(
      data = splines_data2,
      aes(x = x, y = y, group = seed_num, size = spline_size),
      colour = colour_vec2[2]),
    mask = ggfx::ch_alpha("rect3")) +
  geom_shape(
    data = rect_subset3,
    aes(x = x, y = y, group = group),
    radius = unit(0.325, "cm"), colour = colour_vec2[2], size = 0.2,
    fill = NA) +
  scale_size_identity() +
  scale_fill_identity() +
